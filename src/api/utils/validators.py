...
def check_at_least_one_parameter_not_none(**parameters) -> None:
    ...
    Raises:
        HTTPException (422): Count of the not None `parameters` greater than 0.

    """
    if len([parameter for parameter in parameters if parameter is not None]) > 0:
        raise HTTPException(
...
def check_only_one_parameter_not_none(**parameters) -> None:
    ...
    """
    if len([parameter for parameter in parameters if parameter is not None]) != 1:
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY,
