...
def validate_monitoring_name(name: str | None) -> str:
    ...
    """
    if not name:
        raise DetailedAiogramError("Name is required. Please enter a name.")
    if len(name) > 100:
        raise DetailedAiogramError("Name is too long. Please enter a shorter name (max 100 characters).")
    return name
...
def validate_monitoring_url(url: str | None) -> str:
    ...
    """
    if not url:
        raise DetailedAiogramError("URL is required. Please enter a URL.")
    try:
        AnyHttpUrl(url)
    except ValidationError:
        raise DetailedAiogramError("Invalid URL. Please enter a valid URL.")
    if len(url) > 2000:
        raise DetailedAiogramError("URL is too long. Please enter a shorter URL (max 2000 characters).")
    return url
